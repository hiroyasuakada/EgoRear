...
class PoseHeatmapMVFEXLightningModel(LightningModule):
    def __init__(
        self,
        model_cfg: dict,
        dataset_type: str,
        ...
        heatmap_estimator_pretrained_stereo_front=None,
        heatmap_estimator_pretrained_stereo_back=None,
        dataset_kwargs: dict = {}
    ):
        super().__init__()
        ...
        self.network = EgoPoseFormerHeatmapMVFEX(**model_cfg)
        ...
        if heatmap_estimator_pretrained_stereo_front:
            self.load_trained_model(self.network.heatmap_estimator_stereo_front, heatmap_estimator_pretrained_stereo_front)

        if heatmap_estimator_pretrained_stereo_back:
            self.load_trained_model(self.network.heatmap_estimator_stereo_back, heatmap_estimator_pretrained_stereo_back)
    ...
    def training_step(self, batch, batch_idx):
        ...
        pred_heatmap_all, frame_feat_all = self.network(img)

        pred_heatmap_init = pred_heatmap_all[0]
        pred_heatmap_refined_all = pred_heatmap_all[0:]

        loss_dict = OrderedDict()
        list_gt_heatmap = [gt_heatmap[:, l] for l in range(V)]
        for id, pred_heatmap_refined in enumerate(pred_heatmap_refined_all):
            list_pred_heatmap_refined = [pred_heatmap_refined[:, l] for l in range(V)]
            loss_id = sum([self.get_loss(x, y) for x, y in zip(list_pred_heatmap_refined, list_gt_heatmap)])
    ...
    def eval_step(self, batch, batch_idx, mode):
        ...
        pred_heatmap_init = pred_heatmap_all[0]
        pred_heatmap_refined_mid = pred_heatmap_all[1:-1]
        pred_heatmap_refined_final = pred_heatmap_all[-1]

        if (mode == "val") or (mode == "test"):
            output_dict = OrderedDict()

            metric_pred_init_stereo_front = self.evaluate(pred_heatmap_init[:, 0:2, :, :, :], gt_heatmap[:, 0:2, :, :, :], "proposal_stereo_front")
            metric_pred_refined_final_stereo_front = self.evaluate(pred_heatmap_refined_final[:, 0:2, :, :, :], gt_heatmap[:, 0:2, :, :, :], "final_stereo_front")
            output_dict.update(metric_pred_init_stereo_front)
            output_dict.update(metric_pred_refined_final_stereo_front)

            if (V == 4) or (V == 3):
                metric_pred_init_stereo_back = self.evaluate(pred_heatmap_init[:, 2:, :, :, :], gt_heatmap[:, 2:, :, :, :], "proposal_stereo_back")
                metric_pred_refined_final_stereo_back = self.evaluate(pred_heatmap_refined_final[:, 2:, :, :, :], gt_heatmap[:, 2:, :, :, :], "final_stereo_back")
                output_dict.update(metric_pred_init_stereo_back)
                output_dict.update(metric_pred_refined_final_stereo_back)

            if mode == "test":
                for id in range(len(pred_heatmap_refined_mid)):
                    pred_heatmap_refined_mid_id = pred_heatmap_refined_mid[id]
                    metric_pred_refined_mid_stereo_front_id =  self.evaluate(pred_heatmap_refined_mid_id[:, 0:2, :, :, :], gt_heatmap[:, 0:2, :, :, :], "mid_{}_stereo_front".format(id))
                    output_dict.update(metric_pred_refined_mid_stereo_front_id)

                    if (V == 4) or (V == 3):
                        metric_pred_refined_mid_stereo_back_id =  self.evaluate(pred_heatmap_refined_mid_id[:, 2:, :, :, :], gt_heatmap[:, 2:, :, :, :], "mid_{}_stereo_back".format(id))
                        output_dict.update(metric_pred_refined_mid_stereo_back_id)
    ...
    def get_anchors_2d_from_hm(self, heatmap):
        ...
        with torch.no_grad():
            ...
            heatmap = heatmap.contiguous().view(B * V, C, H, W)
